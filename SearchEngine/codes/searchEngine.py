...
def interface():
	def interface():
		...
		logo = tk.Label(root, text='Kaigle', fg="black", font="Calibri 64 bold")
		var = tk.StringVar()
		query_ety = tk.Entry(root, textvariable=var)
		results_output0 = tk.Message(root, text='', width=500)
		results_output1 = tk.Label(root, text="", fg='red',
								   font='Times 20 bold')
		results_output2 = tk.Label(root, text="", fg='dark blue')
		results_output3 = tk.Label(root, text="", fg='black')
		results_output4 = tk.Label(root, text="", fg='red',
								   font='Times 20 bold')
		results_output5 = tk.Label(root, text="", fg='dark blue')
		results_output6 = tk.Label(root, text="", fg='black')
		results_output7 = tk.Label(root, text="", fg='red',
								   font='Times 20 bold')
		results_output8 = tk.Label(root, text="", fg='dark blue')
		results_output9 = tk.Label(root, text="", fg='black')
		results_output10 = tk.Label(root, text="", fg='red',
									font='Times 20 bold')
		results_output11 = tk.Label(root, text="", fg='dark blue')
		results_output12 = tk.Label(root, text="", fg='black')
		results_output13 = tk.Label(root, text="", fg='red',
									font='Times 20 bold')
		results_output14 = tk.Label(root, text="", fg='dark blue')
		results_output15 = tk.Label(root, text="", fg='black')

		search_btn = tk.Button(root, text='Search',command=lambda: print_result(var,results_output0,results_output1,results_output2,results_output3,results_output4,results_output5,results_output6,results_output7,results_output8,results_output9,results_output10,results_output11,results_output12,results_output13,results_output14,results_output15))
	...
	if query == '':
		...
		for documment in Documents:
			if Documents[documment]['num'] == M :
				...
				posWeight = 0
				scores[documment] = parameter[0] * cosin + parameter[1] * headWeight + parameter[4] * pagerank

		for documment in CapitalDocuments:
			if documment not in scores:
				scores[documment] = 0
			scores[documment] += parameter[2] * CapitalDocuments[documment]
			...
			for i in range(N2):
				...
				for j in range(len(position0)):
					...
					if (curPos + 1) in posHash or (curPos + 2) in posHash:
						posWeight += 1
				if posWeight > 0:
					scores[documentName] += parameter[3] * np.log10(1 + posWeight)
			results_raw = sorted(scores.iteritems(), key=lambda d: -d[1])
		for i in range(5):
...
if __name__ == '__main__':
	...
	GoogleResults["information retrieval"] = {'5/451': 5, '19/21': 4, '61/433': 3, '60/155': 3, '51/465': 3,
											  ...
											  '20/58': 1}
	indexPathBase = "C:/Users/admin/Desktop/project3/results/"
	filePathBase = "C:/Users/admin/Desktop/project3/WEBPAGES_CLEAN/"
	...
	for i in range(1, 5):
		y[i] += y[i - 1]
	standard = y
	urlPR_dict = get_json('C:/Users/admin/Desktop/project3/results/sorted_fullurl_PR_dict.json')
