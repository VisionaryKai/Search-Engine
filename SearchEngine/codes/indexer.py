...
def indexer():
    ...
    global res
    res=[]
    MapFile = "C:/Users/admin/Desktop/project3/WEBPAGES_CLEAN/bookkeeping.json"
    ...
    for term in indexdict:
        if len(term[0])>2:
            if (term[0][0]+term[0][1]+term[0][2])!=initial:
                if len(cur)>0:
                    with open('C:/Users/admin/Desktop/project3/results'+initial[0]+'/'+initial[1]+'/'+initial[2]+'.json', 'w') as outfile:
            ...
            if not (term[0][0].isalpha() or term[0][0].isdigit()):
                print('problem')
                z = input()
            with open('C:/Users/admin/Desktop/project3/results' + term[0] + '.json', 'w') as outfile:
